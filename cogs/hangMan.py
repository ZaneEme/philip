...
class hangMan(commands.Cog):
    ...
    async def hangman(self, ctx):
        ...
        gameEmbedOne.add_field(name = "Word progress", value = f"{', '.join(tempUnderscoreArray)}", inline = False)
        ...
        while gameRunning == True:
            ...
            for a in range(0, len(chosenWord)): #if the word contains the guessed character replace the underscores
                if chosenWord[a] == guessedChar:
                    ...
                    gameEmbedOne.set_field_at(index = 0, name = "word progress", value = f"{', '.join(tempUnderscoreArray)}", inline = False)
            ...
            if guessesLeft == 0: #test if all guesses have been used
                gameEmbedOne.set_field_at(index = 1, name = "Information", value = f"You have {guessesLeft} guesses left.\nThe correct word was {chosenWord}.", inline = False)
            ...
            if wordsMatch == True: #if the guessed word matches the chosen word after the full for loop above, end game
                ...
                gameEmbedOne.set_field_at(index = 0, name = "word progress", value = f"{' '.join(tempUnderscoreArray)}", inline = False)
                gameEmbedOne.set_field_at(index = 1, name = "Information", value = f"Good job! You guessed the word correctly with {guessesLeft} guesses remaining.", inline = False)
                gameEmbedOne.remove_field(index = 2)
                gameRunning = False
                await ctx.send(embed = gameEmbedOne)
                break
